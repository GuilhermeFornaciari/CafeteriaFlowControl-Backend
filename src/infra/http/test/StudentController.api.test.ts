...
test("Deve testar o GetOne da entidade Student", async() => {
    ...
    const getStudent = await axios.get('http://localhost:4000/student' + '/' + newStudent.registration + '/' + newStudent.organizationId,
    {
...
test("Deve testar o GetAll da entidade Student", async() => {
  ...
  const newLoginInApi = await loginInApi()
  const getStudent = await axios.get('http://localhost:4000/student' + '/' + students.organizationId,
  {
    headers: {authorization: newLoginInApi.token}
  })
